...
i32 main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		lightPos.x = cos(currentFrame) + sin(currentFrame);
		lightPos.y = 0.0f;
		lightPos.z = cos(currentFrame) - sin(currentFrame);
		...
		lightingShader->setVec3("xyzLht.ambient", ambientColor);
		lightingShader->setVec3("xyzLht.diffuse", diffuseColor);
		lightingShader->setVec3("xyzLht.specular", 1.0f, 1.0f, 1.0f);

		lightingShader->setVec3("xyzMat.ambient", 1.0f, 0.5f, 0.31f);
		lightingShader->setVec3("xyzMat.diffuse", 1.0f, 0.5f, 0.31f);
		lightingShader->setVec3("xyzMat.specular", 0.5f, 0.5f, 0.5f);
		lightingShader->setF32("xyzMat.shininess", 32.0f);

		lightingShader->setMat4("proj", proj);
		lightingShader->setMat4("view", cam->getViewM4());

		glm::mat4 model = glm::mat4(1.0f);
		model = glm::rotate(model, currentFrame*1.5f, { 1.0f, 0.0f, 0.0f });
		lightCubeShader->setMat4("model", model);
		glDrawElements(GL_TRIANGLES, cubex->getISize(), GL_UNSIGNED_INT, 0);

		glBindVertexArray(lightCubeVao);
		lightCubeShader->useProgram();
		lightCubeShader->setVec3("xyzColor", lightColor);
