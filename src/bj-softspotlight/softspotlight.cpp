...
i32 main() {
	...
	Shader* shader      = new Shader("softspotlight.vert", "softspotlight.frag");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shader->setVec3("xyzLht.position", cam->getPos());
		shader->setVec3("xyzLht.direction", cam->getFront());
		shader->setF32("xyzLht.cutOff", cutOff);
		shader->setF32("xyzLht.outerCutOff", outerCutOff);
